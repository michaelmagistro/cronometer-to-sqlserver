...
-- This file only creates or alters the loading procedures. To execute the loading, run: `EXEC dbo.Load_All_Cronometer_Data @base_path = 'C:\exports\cronometer\';` for the main loading procedure OR any of the sub procedures if needed.
...
-- Load Custom Recipes
-- =============================================
-- headers should be: recipeLoopNum,title,ingredient,database,amount,unit,energy (kcal),weight,serving name,servings per recipe
-- in this case, we do NOT drop the table. instead we append.
-- we will have a separate proc to clear duplicate recipe signatures.
CREATE OR ALTER PROCEDURE dbo.Load_Custom_Recipes @base_path NVARCHAR(500)
AS
BEGIN
	SET NOCOUNT ON;

	-- 1. Create the table ONLY if it does not exist
	IF OBJECT_ID('dbo.Custom_Recipes', 'U') IS NULL
	BEGIN
		CREATE TABLE dbo.Custom_Recipes (
			[CreateDate] DATETIME2 NOT NULL DEFAULT GETDATE(),
			[recipeLoopNum] INT NOT NULL,
			[title] NVARCHAR(200) NOT NULL,
			[ingredient] NVARCHAR(200) NOT NULL,
			[database] NVARCHAR(100) NOT NULL,
			[amount] NVARCHAR(20) NOT NULL,
			[unit] NVARCHAR(50) NOT NULL,
			[energy (kcal)] FLOAT NOT NULL,
			[weight] NVARCHAR(50) NOT NULL,
			[serving name] NVARCHAR(100) NOT NULL,
			[servings per recipe] FLOAT NOT NULL
		);
	END

	-- 2. Prepare the file path
	DECLARE @file_path NVARCHAR(1000) = @base_path + N'custom_recipes.csv';
	DECLARE @sql NVARCHAR(MAX);

	-- 3. Execute BULK INSERT (Appends to existing data)
	SET @sql = N'BULK INSERT dbo.Custom_Recipes FROM ''' + @file_path + N''' WITH (' +
		N'FORMAT = ''CSV'',' +
		N'FIRSTROW = 2,' +
		N'FIELDTERMINATOR = '','',' +
		N'ROWTERMINATOR = ''0x0a'',' +
